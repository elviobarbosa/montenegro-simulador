...
add_filter('rest_authentication_errors', function($result) {
    // Se já houver um erro, liberar namespaces que precisam funcionar sem login
    if (true === is_wp_error($result)) {
        $uri = $_SERVER['REQUEST_URI'];
        if (
            strpos($uri, '/wp-json/cvcrm/v1') !== false ||
            strpos($uri, '/wp-json/contact-form-7/') !== false
        ) {
            return null;
        }
    }
...
<div id="<?php echo $map_id; ?>"
  style="background-color: #eaeaea; height: <?php echo esc_attr($atts['height']); ?>; width: <?php echo esc_attr($atts['width']); ?>;">
  <div style="display:flex; align-items: center; justify-content: center; width: 100%; height: 100%"><img
      src="<?php echo plugins_url( 'images/loading.gif', __FILE__ ); ?>"></div>
</div>
<script>
...
async function loadEmpreendimentos(id) {
  try {
    ...
    const res = await fetch(`/wp-json/cvcrm/v1/empreendimentos/${id}?limite_dados_unidade=200`);
...
async function loadTabelaPreco() {
  ...
  try {
    const res = await fetch(
      `/wp-json/cvcrm/v1/tabelas/${tabelaPrecoConfig.idEmpreendimento}/${tabelaPrecoConfig.idTabela}`);
...
async function loadUnidadeValor(empreendimentoId, idunidade, bloco = null) {
  ...
  try {
    ...
    if (data && data.dados && Array.isArray(data.dados)) {
      const unidade = data.dados.find(u => String(u.idunidade) === String(idunidade) || String(u.idunidade_int) ===
        String(idunidade));
...
function infoWindowTemplate(lote) {
  ...
  const buttonBadge = disponivel ?
    `<button class="button button--primary button--normal" data-js="start">SIMULAR AGORA</button>` : '';
    ...
    const send = (isWapp = false) => {
      ...
      google.maps.event.addListenerOnce(infoWindow, "domready", function() {
        ...
        if (formEl && typeof wpcf7 !== "undefined") {
          ...
          const hiddenFields = [{
              name: 'empreendimento',
              value: empreedimentosData.nome || ''
            },
            {
              name: 'quadra',
              value: unidade.idbloco || ''
            },
            {
              name: 'lote',
              value: unidade.nome || ''
            ...
            {
              name: 'valor-lote',
              value: formatCurrency(simulacao.preco)
            },
            {
              name: 'entrada',
              value: formatCurrency(simulacao.entrada)
            },
            {
              name: 'valor-financiado',
              value: formatCurrency(simulacao.financiado)
            },
            {
              name: 'balao-anual',
              value: `${formatCurrency(simulacao.valorBalao)} x ${simulacao.qtdBaloes}`
            },
            {
              name: 'parcelas',
            ...
            {
              name: 'valor-parcela',
              value: formatCurrency(simulacao.valorParcela)
            }
          ];
          ...
          document.addEventListener('wpcf7mailsent', function(event) {
            ...
            mensagem += `--------------------------\n\n`;
            mensagem +=
              `Valor do Lote: ${Number(simulacao.preco).toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}\n`;
            mensagem +=
              `Entrada: ${simulacao.entrada.toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}\n`;
            mensagem +=
              `Valor Financiado: ${simulacao.financiado.toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}\n`;
            mensagem +=
              `Balão Anual (20%): ${simulacao.valorBalao.toLocaleString("pt-BR",{style:"currency",currency:"BRL"})} x ${simulacao.qtdBaloes}\n`;
            mensagem += `Qtd. Parcelas: ${simulacao.parcelas}\n`;
            mensagem +=
              `Valor da Parcela Mensal: ${simulacao.valorParcela.toLocaleString("pt-BR",{style:"currency",currency:"BRL"})}\n`;

            const url =
              `https://wa.me/55${phone.replace(/\D/g, "")}?text=${encodeURIComponent(mensagem)}`;
    ...
    if (lote.coordinates && lote.coordinates.length > 0 && lote.bloco) {
      ...
      polygon.addListener('click', async function(event) {
        ...
        infoWindow.setContent(
          '<div class="info-window-step2" style="text-align:center;padding:20px;">Carregando...</div>');
        ...
        if (dadosTabela) {
          ...
          // Fallback: busca no endpoint antigo
          const valorApi = await loadUnidadeValor(<?php echo $empreendimento_id; ?>, unidade.idunidade, lote
            .bloco);
          if (valorApi !== null) {
            unidade.valor = valorApi;
          }
...
class NumberSlider {
  ...
  init() {
    ...
    const formatCurrency = (valor) => valor.toLocaleString("pt-BR", {
      style: "currency",
      currency: "BRL"
    });
    ...
    const updateSliderStyle = (slider) => {
      ...
      slider.style.background =
        `linear-gradient(to right, #316D78 0%, #316D78 ${percent}%, #ddd ${percent}%, #ddd 100%)`;
    };
